...
def metric_report(data_rank, topk=10):
    ...
    hit_sum = {k: 0 for k in K_list}

    for rank in data_rank:

        for k in K_list:
            if rank < k:
                ndcg_sum[k] += 1 / np.log2(rank + 2)
                hit_sum[k] += 1

    total = len(data_rank)
    return {
        'NDCG@5': ndcg_sum[5] / total if total != 0 else 0,
        'HR@5': hit_sum[5] / total if total != 0 else 0,
        'NDCG@10': ndcg_sum[10] / total if total != 0 else 0,
        'HR@10': hit_sum[10] / total if total != 0 else 0,
        'NDCG@20': ndcg_sum[20] / total if total != 0 else 0,
        'HR@20': hit_sum[20] / total if total != 0 else 0,
        'NDCG@50': ndcg_sum[50] / total if total != 0 else 0,
        'HR@50': hit_sum[50] / total if total != 0 else 0,
    }
...
def metric_len_report(data_rank, data_len, topk=10, aug_len=0, args=None):
    ...
    K_list = [5, 10, 20, 50]
    ndcg_s_sum = {k: 0.0 for k in K_list}
    hit_s_sum = {k: 0 for k in K_list}
    ndcg_l_sum = {k: 0.0 for k in K_list}
    hit_l_sum = {k: 0 for k in K_list}
    ...
    for i, rank in enumerate(data_rank):
        ...
        if rank < 50:  # micro optimization; still correct for k \in {5,10,20,50}
            if data_len[i] < ts_user + aug_len:
                for k in K_list:
                    if rank < k:
                        ndcg_s_sum[k] += 1 / np.log2(rank + 2)
                        hit_s_sum[k] += 1
            else:
                for k in K_list:
                    if rank < k:
                        ndcg_l_sum[k] += 1 / np.log2(rank + 2)
                        hit_l_sum[k] += 1

    return {
        'Short NDCG@5': ndcg_s_sum[5] / count_s if count_s != 0 else 0,
        'Short HR@5': hit_s_sum[5] / count_s if count_s != 0 else 0,
        'Long NDCG@5': ndcg_l_sum[5] / count_l if count_l != 0 else 0,
        'Long HR@5': hit_l_sum[5] / count_l if count_l != 0 else 0,
        'Short NDCG@10': ndcg_s_sum[10] / count_s if count_s != 0 else 0,
        'Short HR@10': hit_s_sum[10] / count_s if count_s != 0 else 0,
        'Long NDCG@10': ndcg_l_sum[10] / count_l if count_l != 0 else 0,
        'Long HR@10': hit_l_sum[10] / count_l if count_l != 0 else 0,
        'Short NDCG@20': ndcg_s_sum[20] / count_s if count_s != 0 else 0,
        'Short HR@20': hit_s_sum[20] / count_s if count_s != 0 else 0,
        'Long NDCG@20': ndcg_l_sum[20] / count_l if count_l != 0 else 0,
        'Long HR@20': hit_l_sum[20] / count_l if count_l != 0 else 0,
        'Short NDCG@50': ndcg_s_sum[50] / count_s if count_s != 0 else 0,
        'Short HR@50': hit_s_sum[50] / count_s if count_s != 0 else 0,
        'Long NDCG@50': ndcg_l_sum[50] / count_l if count_l != 0 else 0,
        'Long HR@50': hit_l_sum[50] / count_l if count_l != 0 else 0,
    }
...
def metric_pop_report(data_rank, pop_dict, target_items, topk=10, aug_pop=0, args=None):
    ...
    return {
        'Tail NDCG@5': ndcg_s_sum[5] / count_s if count_s != 0 else 0,
        'Tail HR@5': hit_s_sum[5] / count_s if count_s != 0 else 0,
        'Popular NDCG@5': ndcg_l_sum[5] / count_l if count_l != 0 else 0,
        'Popular HR@5': hit_l_sum[5] / count_l if count_l != 0 else 0,
        'Tail NDCG@10': ndcg_s_sum[10] / count_s if count_s != 0 else 0,
        'Tail HR@10': hit_s_sum[10] / count_s if count_s != 0 else 0,
        'Popular NDCG@10': ndcg_l_sum[10] / count_l if count_l != 0 else 0,
        'Popular HR@10': hit_l_sum[10] / count_l if count_l != 0 else 0,
        'Tail NDCG@20': ndcg_s_sum[20] / count_s if count_s != 0 else 0,
        'Tail HR@20': hit_s_sum[20] / count_s if count_s != 0 else 0,
        'Popular NDCG@20': ndcg_l_sum[20] / count_l if count_l != 0 else 0,
        'Popular HR@20': hit_l_sum[20] / count_l if count_l != 0 else 0,
        'Tail NDCG@50': ndcg_s_sum[50] / count_s if count_s != 0 else 0,
        'Tail HR@50': hit_s_sum[50] / count_s if count_s != 0 else 0,
        'Popular NDCG@50': ndcg_l_sum[50] / count_l if count_l != 0 else 0,
        'Popular HR@50': hit_l_sum[50] / count_l if count_l != 0 else 0,
    }
...
def load_pretrained_model(pretrain_dir, model, logger, device):
    ...
    # log the number of parameters updated
