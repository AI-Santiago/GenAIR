...
from models.GenAIR import GenAIRSASRec, GenAIRBert4Rec, GenAIRGRU4Rec
...
class SeqTrainer(Trainer):
    ...
    def _create_model(self):
        ...
        ## Our Method -- GenAIR
        elif self.args.model_name == "genair_sasrec":
            self.model = GenAIRSASRec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "genair_bert4rec":
            self.model = GenAIRBert4Rec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "genair_gru4rec":
            self.model = GenAIRGRU4Rec(self.user_num, self.item_num, self.device, self.args)
    ...
    def save_user_emb(self):
        ...
        save_dir = os.path.join("./data", self.args.dataset, "handled")
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, "usr_emb_{}.pkl".format(self.args.model_name))
        pickle.dump(user_emb, open(save_path, "wb"))
    ...
    def save_item_emb(self):
        ...
        save_dir = os.path.join("./data", self.args.dataset, "handled")
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, "itm_emb_{}.pkl".format(self.args.model_name))
